...
class Agent():
    ...
    def __init__(self,
        ...
        if self.distributional:
            if self.D2RL:
                ...
                self.critic1_target = DeepIQN(state_size, action_size, hidden_size, random_seed, self.N, device).to(
                    device)
                self.critic1_target.load_state_dict(self.critic1.state_dict())

                self.critic2_target = DeepIQN(state_size, action_size, hidden_size, random_seed, self.N, device).to(
                    device)
        ...
        with torch.no_grad():
            ...
            if not self.munchausen:
                if self.FIXED_ALPHA == None:
                    ...
                    Q_targets = rewards.cpu() + (gamma ** self.n_step * (1 - dones.cpu()) * (
                                Q_target_next.cpu() - self.alpha * log_pis_next.cpu()))
                else:
                    Q_targets = rewards.cpu() + (gamma ** self.n_step * (1 - dones.cpu()) * (
                                Q_target_next.cpu() - self.FIXED_ALPHA * log_pis_next.cpu()))
            else:
                ...
                if self.FIXED_ALPHA == None:
                    # Compute Q targets for current states (y_i)
                    Q_targets = munchausen_reward + (gamma ** self.n_step * (1 - dones.cpu()) * (
                                Q_target_next.cpu() - self.alpha * log_pis_next.cpu()))
                else:
                    Q_targets = munchausen_reward + (gamma ** self.n_step * (1 - dones.cpu()) * (
                                Q_target_next.cpu() - self.FIXED_ALPHA * log_pis_next.cpu()))
    ...
    def learn_per(self, step, experiences, gamma, d=1):
            ...
            if not self.munchausen:
                if self.FIXED_ALPHA == None:
                    ...
                    Q_targets = rewards.cpu() + (
                                gamma * (1 - dones.cpu()) * (Q_target_next.cpu() - self.alpha * log_pis_next.cpu()))
                else:
                    Q_targets = rewards.cpu() + (gamma * (1 - dones.cpu()) * (
                                Q_target_next.cpu() - self.FIXED_ALPHA * log_pis_next.cpu()))
            else:
                mu_m, log_std_m = self.actor_local(states)
                std = log_std_m.exp()
                ...
                assert log_pi_a.shape == (self.batch_size, 1)
                munchausen_reward = (
                            rewards.cpu() + self.m_alpha * torch.clamp(self.m_tau * log_pi_a, min=self.lo, max=0))
                assert munchausen_reward.shape == (self.batch_size, 1)
                if self.FIXED_ALPHA == None:
                    # Compute Q targets for current states (y_i)
                    Q_targets = munchausen_reward + (
                                gamma * (1 - dones.cpu()) * (Q_target_next.cpu() - self.alpha * log_pis_next.cpu()))
                else:
                    Q_targets = munchausen_reward + (gamma * (1 - dones.cpu()) * (
                                Q_target_next.cpu() - self.FIXED_ALPHA * log_pis_next.cpu()))
        ...
        if step % d == 0:
            ...
            alpha_loss = (- (
                        self.log_alpha.exp() * (log_pis.cpu() + self.target_entropy).detach().cpu()) * weights).mean()
            self.alpha_optimizer.zero_grad()
            alpha_loss.backward()
            self.alpha_optimizer.step()
            self.alpha = self.log_alpha.exp().detach()
    ...
    def learn_distr(self, step, experiences, gamma, d=1):
        ...
        with torch.no_grad():
            ...
            if not self.munchausen:
                if self.FIXED_ALPHA == None:
                    # Compute Q targets for current states (y_i)
                    Q_targets = rewards.cpu().unsqueeze(-1) + (gamma * (1 - dones.cpu().unsqueeze(-1)) * (
                                Q_target_next.cpu() - self.alpha * log_pis_next.cpu().unsqueeze(-1)))
                else:
                    Q_targets = rewards.cpu().unsqueeze(-1) + (gamma * (1 - dones.cpu().unsqueeze(-1)) * (
                                Q_target_next.cpu() - self.FIXED_ALPHA * log_pis_next.cpu().unsqueeze(-1)))
            else:
                mu_m, log_std_m = self.actor_local(states)
                std = log_std_m.exp()
                dist = Normal(mu_m, std)
                ...
                assert log_pi_a.shape == (self.batch_size, 1)
                munchausen_reward = (
                            rewards.cpu() + self.m_alpha * torch.clamp(log_pi_a, min=self.lo, max=0)).unsqueeze(-1)
                assert munchausen_reward.shape == (self.batch_size, 1, 1)
                if self.FIXED_ALPHA == None:
                    # Compute Q targets for current states (y_i)
                    Q_targets = munchausen_reward + (gamma * (1 - dones.cpu().unsqueeze(-1)) * (
                                Q_target_next.cpu() - self.alpha * log_pis_next.cpu().unsqueeze(-1)))
                else:
                    Q_targets = munchausen_reward + (gamma * (1 - dones.cpu().unsqueeze(-1)) * (
                                Q_target_next.cpu() - self.FIXED_ALPHA * log_pis_next.cpu().unsqueeze(-1)))
